...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in legalActions:
            if self.getQValue(state, action) > max_value:
                ...
                max_action = action
        return max_value if max_action is not None else 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legalActions:
            if self.getQValue(state, action) > max_value:
                max_value = self.getQValue(state, action)
                max_action = action
        return max_action if max_action is not None else None
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.Qdict[(state, action)] += self.alpha * (reward + (self.discount * self.getValue(nextState)) - self.Qdict[(state, action)])
